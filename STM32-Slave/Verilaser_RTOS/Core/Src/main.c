...
/**
  ******************************************************************************
  * @file           : main.c
  * @brief          : Main program body (조이스틱 서보 테스트)
  ******************************************************************************
  */
...
/* Definitions for Task_I2C (조이스틱으로 대체) */
...
/* ── Queue: sizeof(Coord_t) 로 수정 ─────────────────────
 *   기존 sizeof(uint16_t) → 구조체 크기 불일치로 데이터 깨짐
 * ──────────────────────────────────────────────────────── */
...
int main(void)
{
  ...
  Task_ServoHandle  = osThreadNew(StartTask_Servo,     NULL, &Task_Servo_attributes);   /* Servo.c */
  Task_LaserHandle  = osThreadNew(StartTask03,         NULL, &Task_Laser_attributes);
  Task_I2CHandle    = osThreadNew(StartTask_Joystick,  NULL, &Task_I2C_attributes);     /* Joystick.c */
...
static void MX_DMA_Init(void)
{
  ...
  HAL_NVIC_SetPriority(DMA2_Stream0_IRQn, 5, 0);   /* FreeRTOS 우선순위 5 이상 */
...
static void MX_ADC1_Init(void)
{
  ...
  hadc1.Init.ScanConvMode          = ENABLE;       /* 다채널 스캔 */
  ...
  /* ── 채널 설정: 실제 핀에 맞게 수정하세요 ──────────────
   *   X축 → 연결된 ADC 채널 번호로 변경
   *   Y축 → 연결된 ADC 채널 번호로 변경
   *   ex) PA0=CH0, PA1=CH1, PA4=CH4, PB0=CH8 등
   * ──────────────────────────────────────────────────── */

  /* X축 (Rank 1) */
  sConfig.Channel      = ADC_CHANNEL_0;    /* ← X축 핀 채널로 수정 */
  sConfig.Rank         = 1;
  sConfig.SamplingTime = ADC_SAMPLETIME_84CYCLES;
  if (HAL_ADC_ConfigChannel(&hadc1, &sConfig) != HAL_OK) Error_Handler();

  /* Y축 (Rank 2) */
  sConfig.Channel      = ADC_CHANNEL_1;    /* ← Y축 핀 채널로 수정 */
  sConfig.Rank         = 2;
  if (HAL_ADC_ConfigChannel(&hadc1, &sConfig) != HAL_OK) Error_Handler();
}
...
// *   (319, 239) ?�� ?��른쪽 ?��?�� ?�� ?���? ?��?��
// * ???????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????? */
//#define TEST_X       0     /* 0 ~ 319 */
//#define TEST_Y       0     /* 0 ~ 239 */
//
...
//  // StartTask04를 StartTask_Joystick으로 변경
...
//static float pan_angle  = 90.0f;   /* Pan  ?��?�� 각도 (0~180) */
//static float tilt_angle = 90.0f;   /* Tilt ?��?�� 각도 (0~180) */
//uint16_t adc_buffer[ADC_BUF_LEN]; // DMA�?? 조이?��?�� 값을 채워�?? 배열
//JoystickData_t joy_data;
///* USER CODE END PV */
//
///* Private function prototypes -----------------------------------------------*/
//void SystemClock_Config(void);
...
//  MX_ADC1_Init();
//  /* USER CODE BEGIN 2 */
////  HAL_ADC_Start_DMA(&hadc1, (uint32_t*)adc_buffer, ADC_BUF_LEN); // ADC ?��?��
////  HAL_TIM_PWM_Start(&htim3, TIM_CHANNEL_1);
////  HAL_TIM_PWM_Start(&htim3, TIM_CHANNEL_2);
////  HAL_TIM_PWM_Start(&htim3, TIM_CHANNEL_1);   /* Pan  : PA6 */
////  HAL_TIM_PWM_Start(&htim3, TIM_CHANNEL_2);   /* Tilt : PA7 */
...
//  HAL_TIM_PWM_Start(&htim3, TIM_CHANNEL_1);   /* Pan  : PA6 */
//  HAL_TIM_PWM_Start(&htim3, TIM_CHANNEL_2);   /* Tilt : PA7 */
//  Servo_SetAngle(90.0f, 90.0f);               /* 초기 중립 ?���?? */
//  HAL_Delay(500);
//  /* USER CODE END 2 */
//
//  /* Init scheduler */
