...
/* Private define ------------------------------------------------------------*/
/* USER CODE BEGIN PD */
#define I2C_SLAVE_ADDR   0x20   /* 7-bit addr=0x10, Ï¢åÏãú?îÑ?ä∏ */
/* USER CODE END PD */
...
volatile uint8_t g_mode = MODE_MANUAL;   /* 0=?àò?èô, 1=?òÅ?†ê, 2=?ûê?èô */
volatile uint8_t g_aim_locked = 0;       /* 1=Ï°∞Ï? ?ôÑÎ£?(1Ï¥? Í≤ΩÍ≥º), 0=Í≤ΩÍ≥† Ï§? */
char uart_buf[128];
/* USER CODE END PV */
...
int main(void)
{
  ...
  /* CubeMXÍ∞? sizeof(uint16_t)Î°? ?Éù?Ñ±?ïòÎØ?Î°? TargetData_t ?Å¨Í∏∞Î°ú ?û¨?Éù?Ñ± */
...
{
  /* USER CODE BEGIN 5 */
  /* defaultTask: Î™®Îìú ?†Ñ?ôò Î≤ÑÌäº + Fire Î≤ÑÌäº Ï≤òÎ¶¨ (LeeJae StartTask03 Ï∞∏Í≥†) */
  uint8_t btn_prev = 1;

  for(;;)
  {
    /* ???? Btn_mode(PB15) ?Ü†Í∏?: Mode 0 ?Üí 1 ?Üí 2 ?Üí 0 ... ???? */
    uint8_t btn_now = HAL_GPIO_ReadPin(Btn_mode_GPIO_Port, Btn_mode_Pin);
    if (btn_prev == 1 && btn_now == 0)  /* falling edge */
    {
      g_mode = (g_mode + 1) % 3;

      /* LED ?ëú?ãú: ?òÅ?†êÎ™®Îìú=ON, ?ÇòÎ®∏Ï?=OFF */
      HAL_GPIO_WritePin(LD2_GPIO_Port, LD2_Pin,
                        (g_mode == MODE_ZEROING) ? GPIO_PIN_SET : GPIO_PIN_RESET);

      osDelay(50);  /* ?îîÎ∞îÏö¥?ã± */
    }
    btn_prev = btn_now;

    /* ???? Btn_fire(PB13) ?Üí Led_fire(PB14) ?ëú?ãú ???? */
...
void Servo_Task(void *argument)
{
  /* USER CODE BEGIN Servo_Task */
	...
	uint8_t prev_mode = 0xFF;  /* Î™®Îìú ?†Ñ?ôò Í∞êÏ??ö© */

  for(;;)
  {
	/* Î™®ÎìúÍ∞? Î∞îÎ?åÎ©¥ ?ÑúÎ≥¥Î?? ?Ñº?Ñ∞Î°? Î¶¨ÏÖã */
	if(g_mode != prev_mode)
	{
		Servo_GoCenter();
		prev_mode = g_mode;
	...
	case MODE_MANUAL:
	{
		...
		/* I2C ?Åê?èÑ ÎπÑÏõå?Ñú Í∞ùÏ≤¥ Í∞êÏ? ?ÉÅ?Éú ?óÖ?ç∞?ù¥?ä∏ (Î∂????ö©) */
...
void Laser_Task(void *argument)
{
  ...
  for(;;)
  {
	...
	{
	case MODE_MANUAL:
		/* ?àò?èô Î™®Îìú: Btn_fire ?àÑÎ•¥Îäî ?èô?ïàÎß? ON */
		if(fire)
			Laser_On();
		else
			Laser_Off();
	...
	case MODE_ZEROING:
		/* ?òÅ?†ê Î™®Îìú: ?ï≠?ÉÅ ON (?Ñ§Ïπ? Í∏∞Ï??†ê ?ôï?ù∏?ö©) */
		Laser_On();
		break;

	case MODE_TRACK:
		/* ?ûê?èô Ï∂îÏ†Å: 1Ï¥? Í≤ΩÍ≥† ?õÑ Ï°∞Ï? ?ôÑÎ£? ?ãú?óêÎß? Î∞úÏÇ¨ */
		if(g_aim_locked)
			Laser_On();
		else
			Laser_Off();
...
void I2C_Task(void *argument)
{
  ...
  /* ?îîÎ≤ÑÍ∑∏ Î°úÍ∑∏ ?†Ñ?ã¥ ?Éú?ä§?Å¨: UART ?Ç¨?ö©?? ?ó¨Í∏∞ÏÑúÎß? */
...
void Buzzer_Task(void *argument)
{
  ...
  for(;;)
  {
	...
	{
	case MODE_MANUAL:
		/* ?àò?èô Î™®Îìú: Í∞ùÏ≤¥ Í∞êÏ? ?ãú ?ó∞?Üç?ùå */
	...
	case MODE_TRACK:
		if(g_target_status == 1)
		{
			...
			if(elapsed < 1000)
			{
				/* Ï≤òÏùå 1Ï¥?: ?Ç°?Ç°?Ç°?Ç° (Í≤ΩÍ≥†, ?†à?ù¥?? ÎØ∏Î∞ú?Ç¨) */
				g_aim_locked = 0;
				Buzzer_Beep(100, 100);
			}
			else
			{
				/* 1Ï¥? ?õÑ: ?ó∞?Üç?ùå + ?†à?ù¥?? Î∞úÏÇ¨ */
				g_aim_locked = 1;
				Buzzer_On();
				osDelay(50);
			}
